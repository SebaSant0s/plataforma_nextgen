...
const channel_state_1 = require("./channel_state");
...
class Channel {
    ...
    constructor(client, type, id, data) {
        ...
        this.state = new channel_state_1.ChannelState(this);
    ...
    deleteReaction(messageID, reactionType, user_id) {
        ...
        const url = this.getClient().baseURL +
    ...
    async updatePartial(update) {
        ...
        const areCapabilitiesChanged = [...(data.channel.own_capabilities || [])].sort().join() !==
            [...(Array.isArray(this.data?.own_capabilities) ? this.data?.own_capabilities : [])].sort().join();
     ...
     */
    async mute(opts = {}) {
        return await this.getClient().post(this.getClient().baseURL + '/moderation/mute/channel', { channel_cid: this.cid, ...opts });
    }
    /**
    ...
    clean() {
        if (this.lastKeyStroke) {
            ...
            if (diff > 1000 && this.isTyping) {
                (0, utils_1.logChatPromiseExecution)(this.stopTyping(), 'stop typing event');
            }
        }
        this.state.clean();
    }
    ...
    async getReplies(parent_id, options, sort) {
        const normalizedSort = sort ? (0, utils_1.normalizeQuerySort)(sort) : undefined;
    ...
    _countMessageAsUnread(message) {
        ...
        if (message.parent_id && !message.show_in_channel)
            return false;
        if (message.user?.id === this.getClient().userID)
            return false;
    ...
    countUnreadMentions() {
        ...
        for (let i = 0; i < this.state.latestMessages.length; i += 1) {
            const message = this.state.latestMessages[i];
            if (this._countMessageAsUnread(message) &&
    ...
    async query(options, messageSetToAddToIfDoesNotExist = 'current') {
        ...
        if (!this.id) {
            ...
            const tempChannelCid = (0, utils_1.generateChannelTempCid)(this.type, state.members.map((member) => member.user_id || member.user?.id || ''));
        ...
        messageSet.pagination = {
            ...
            ...(0, utils_1.messageSetPagination)({
                ...
                requestedPageSize: options?.messages?.limit ?? constants_1.DEFAULT_QUERY_CHANNEL_MESSAGE_LIST_PAGE_SIZE,
    ...
    async createCall(options) {
        return await this.getClient().post(this._channelURL() + '/call', options);
    }
    /**
    ...
    _handleChannelEvent(event) {
        ...
        switch (event.type) {
            ...
            case 'member.updated':
                ...
                if (typeof channelState.membership.user?.id === 'string' &&
