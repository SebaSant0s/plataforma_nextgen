...
import { MessageWebhookListener } from '../../components/MessageWebhookListener';

import type { Channel, Message, SendFileAPIResponse } from '../../../../stream-chat/src';
...
// Custom upload handlers added from CustomMessageInput
export const doFileUploadRequest = async (
  file: LocalAttachmentUploadMetadata['file'],
  channel: Channel,
): Promise<SendFileAPIResponse> => {
  try {
    ...
    const response = await fetch('api/sent/', { method: 'POST', body: formData });
    if (!response.ok) throw new Error('File upload failed');

    const data = await response.json();
    return { file: data.fileUrl, duration: '0' };
  } catch (error) {
...
export const doImageUploadRequest = async (
  file: LocalAttachmentUploadMetadata['file'],
  channel: Channel,
): Promise<SendFileAPIResponse> => {
  try {
...
>(
  ...
  const ListenerComponent = <MessageWebhookListener />;
