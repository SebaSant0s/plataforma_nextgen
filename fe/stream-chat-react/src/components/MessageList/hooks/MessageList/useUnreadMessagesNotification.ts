...
export const useUnreadMessagesNotification = ({
  ...
  useEffect(() => {
    ...
    const panels = document.getElementsByClassName(MESSAGE_LIST_MAIN_PANEL_CLASS);
    const msgListPanel = panels.length > 0 ? panels[0] : null;
    if (!msgListPanel) return;
  
    const separators = document.getElementsByClassName(UNREAD_MESSAGE_SEPARATOR_CLASS);
    const observedTarget = separators.length > 0 ? separators[0] : null;
    if (!observedTarget) {
      setShow(true);
      return;
    }
