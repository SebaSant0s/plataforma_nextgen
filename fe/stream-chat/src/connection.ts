...
export class StableWSConnection<StreamChatGenerics extends ExtendableGenerics = DefaultGenerics> {
  ...
  async connect(timeout = 15000) {
    ...
    try {
      ...
      const err = error as Error & { code?: string | number; isWSFailure?: boolean; StatusCode?: string | number };

      if (err.code === chatCodes.TOKEN_EXPIRED && !this.client.tokenManager.isStatic()) {
      ...
      (async () => {
        ...
        for (let i = 0; i <= timeout; i += interval) {
          try {
            ...
            const err = error as Error & { code?: string | number; StatusCode?: string | number; isWSFailure?: boolean };
            if (i === timeout) {
              throw new Error(
                JSON.stringify({
                  code: err.code,
